...
' do
  context 'Existing ticker' do
    ...
    given(:stock_new_quote) { File.read("#{Rails.root}/spec/data/stock_new_quote.json") }
    ...
    scenario 'updates quote', js: true do
      ...
      allow(Stock::FetchData).to receive(:call).and_return({ stock_json: stock_new_quote, status: 200, source: 'tradier' })
